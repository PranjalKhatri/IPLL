    ...
    if (!sl.isComment) {
      ...
      if (isFirstMne) {
        ...
        if (strlen(sl.label) > PROG_NAME_MAX_LENGTH) {
          printf("Program name %s exceeds maximum allowed name length "
                 "of %d\n",
      ...
      if (mne == END) {
        ...
        if (!ht_get(symbolTable, sl.args[0], &out)) {
          printf("Label defined with END %s not found in the "
                 "symbolTable\n",
                 sl.args[0]);
          retValue = 7;
          goto exit;
...
void DumpHeader(FILE *fp, const char *progName, size_t programLength,
                size_t FirstInstruction) {
  fprintf(fp, "H%-6.6s%06zX%06zX\n", progName ? progName : "", FirstInstruction,
          programLength);
}
void DumpEndRecord(FILE *fp, size_t FirstInstruction) {
  fprintf(fp, "E%06zx\n", FirstInstruction);
...
void DumpLex(const char *const fname, SourceLine *sourceLines,
             size_t numLines) {
  if (fname == NULL || sourceLines == NULL)
    return;
...
#define INST_WIDTH 8
  ...
  // label (or empty)
  fprintf(fp, "%-*s ", LABEL_WIDTH, ol->source->label ? ol->source->label : "");
  ...
  for (int j = 0; j < 3 && ol->source->args[j] != NULL; j++) {
    ...
    if (j < 2 && ol->source->args[j + 1] != NULL)
      fprintf(fp, ",");
  }
...
void DumpIntermediate(const char *const fname, ObjectCodeLine *objectCodeLines,
                      size_t numLines) {
  if (fname == NULL || objectCodeLines == NULL)
    return;
...
int bytes_needed_unsigned(unsigned long long n) {
  ...
  if (n == 0)
    return 1; /* zero still needs 1 byte */
